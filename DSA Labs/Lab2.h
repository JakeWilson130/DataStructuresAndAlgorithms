...
public:
	...
	bool Contains(int _val) const {
		...
		if (count(mValues.begin(), mValues.end(), _val))
		{
			return true;
		}
		return false;
	...
	void MovePalindromes() {	
		...
		for (size_t i = 0; i < mValues.size(); i++)
		{
				...
				{
					mPalindromes.insert(mPalindromes.end(), std::make_move_iterator(mValues.begin() + i), std::make_move_iterator(mValues.end()));
					mValues.erase(mValues.begin() + i, mValues.end());
